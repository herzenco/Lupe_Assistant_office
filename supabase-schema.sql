...
-- Content Asset Bridge: local media registry and temporary public exposure audit
...
CREATE INDEX idx_content_assets_relative_path
  ON content_assets(relative_path);

CREATE INDEX idx_content_assets_created
...
CREATE TABLE content_asset_exposures (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_content_asset_exposures_asset
  ON content_asset_exposures(asset_id, exposed_at DESC);

CREATE INDEX idx_content_asset_exposures_token
  ON content_asset_exposures(token);

CREATE INDEX idx_content_asset_exposures_cleanup
